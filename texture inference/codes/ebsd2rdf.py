...
KGS_DIR = CODES_DIR.parent / "KGs"
ABOX_DIR = KGS_DIR / "abox"
TBOX_DIR = KGS_DIR / "tbox"
MERGED_DIR = KGS_DIR / "merged"
REASONED_DIR = KGS_DIR / "reasoned"
# ROBOT-extracted, chain-stripped TBox module used for Konclude runs -- see
# KGs/tbox/generate_minimal_tbox.sh.
DEFAULT_TBOX_PATH = TBOX_DIR / "minimal_module.owl"
...
DEFAULT_CSV = DATA_DIR / "grain_misorientation.csv"
# The full ABox -- every grain's crystallite + area -- never fed to
# owlready2 (no reasoning needed for area fractions). ABox-only, no TBox
# axioms in it, despite the "crystallites_ttl" variable name shared with the
# TBox-loading reasoner code elsewhere in this file.
DEFAULT_CRYSTALLITES_TTL = ABOX_DIR / "crystallites_full.ttl"
...
DEFAULT_TEXTURE_TTL = ABOX_DIR / "crystallites_texture.ttl"
DEFAULT_REASONED_TTL = REASONED_DIR / "crystallites_reasoned_abox.ttl"
...
# side by side.
DEFAULT_DIRECT_CLASSIFIED_TTL = REASONED_DIR / "crystallites_classified_direct.ttl"
DEFAULT_OUTPUT = REASONED_DIR / "ebsd_texture_inferred.ttl"

SPARQL_DIR = CODES_DIR.parent / "sparql"
CRYSTALLITE_QUERY_FILE = SPARQL_DIR / "crystallite_orientation.rq"
GRAIN_POPULATION_QUERY_FILE = SPARQL_DIR / "grain_population.rq"
TEXTURE_FRACTION_QUERY_FILE = SPARQL_DIR / "texture_fraction.rq"
...
def _qualifying_angle(source: rdflib.Graph, miso: rdflib.term.Node) -> bool:
	"""True if `miso` (a misorientation-angle individual) has a numeric
	value at or under MISORIENTATION_ANGLE_TOLERANCE. Shared by filter_semi()
	and filter_full()."""
	svs = next(source.objects(miso, SPECIFIED_BY_VALUE), None)
	if svs is None:
		return False
	angle = next(source.objects(svs, HAS_NUMERIC_VALUE), None)
	return angle is not None and float(angle) <= MISORIENTATION_ANGLE_TOLERANCE


def _copy_relation(kept: rdflib.Graph, source: rdflib.Graph, grain: rdflib.term.Node, miso: rdflib.term.Node) -> None:
	"""Copies one (grain, misorientation-angle) relation -- the crystallite
	type triple, the PMD_0025998 edge, and the miso/SVS individuals' own
	predicate-objects -- from `source` into `kept`. Shared by filter_semi()
	and filter_full()."""
...
def filter_semi(source: rdflib.Graph) -> rdflib.Graph:
	"""Grain-level Konclude-ABox filter: keeps ALL 8 of a grain's relations,
	for any grain that has at least one misorientation angle satisfying the
	ontology's <=15deg tolerance restriction. This is exactly
	--relevant-only's semantics (see CrystalliteBuilder._stage), just applied
	as a standalone post-hoc filter on an already-built (non---relevant-only)
	texture_ttl rather than requiring a Stage A rerun with the flag set."""
	qualifying_grains = {
		grain
		for grain, _, miso in source.triples((None, HAS_RELATIONAL_QUALITY, None))
		if _qualifying_angle(source, miso)
	}
	kept = rdflib.Graph()
	for grain, _, miso in source.triples((None, HAS_RELATIONAL_QUALITY, None)):
		if grain in qualifying_grains:
			_copy_relation(kept, source, grain, miso)
	return kept


def filter_full(source: rdflib.Graph) -> rdflib.Graph:
	"""Pair-level Konclude-ABox filter: keeps only the individual (grain,
	ideal-texture) PAIRS that satisfy the tolerance -- unlike filter_semi(),
	drops a qualifying grain's other, non-qualifying relations too. This is
	the strictest ("fully filtered") variant (1644 of 18320 pairs on the
	full 2290-grain dataset)."""
	kept = rdflib.Graph()
	for grain, _, miso in source.triples((None, HAS_RELATIONAL_QUALITY, None)):
		if _qualifying_angle(source, miso):
			_copy_relation(kept, source, grain, miso)
...
FILTER_LEVELS = {"semi": filter_semi, "full": filter_full}


def extract_grain_type_assertions(konclude_owl_path: str) -> List[Tuple[str, str]]:
	"""Parses a Konclude `realisation` output (OWL/XML, ClassAssertion facts
	only -- no area/property data, see Konclude's own docs: "writes the
	types of the requested individuals ... into an OWL 2 ontology") and
	returns the (individual_iri, class_iri) pairs for just the 8 GRAIN_TYPES
	classes. Used by EBSD2RDFPipeline.merge_konclude_results() to merge
	those facts back into a full ABox -- Konclude's own equivalent of what
	HermitReasoner.classify() + reason() already do automatically for
	HermiT."""
	grain_type_iris = {str(gt) for gt in GRAIN_TYPES}
	tree = ET.parse(konclude_owl_path)
	root = tree.getroot()
	ns = {"owl": "http://www.w3.org/2002/07/owl#"}

	found = []
	for ca in root.findall(".//owl:ClassAssertion", ns):
		cls = ca.find("owl:Class", ns)
		ind = ca.find("owl:NamedIndividual", ns)
		if cls is None or ind is None:
			continue
		cls_iri = cls.attrib.get("IRI")
		ind_iri = ind.attrib.get("IRI")
		if cls_iri in grain_type_iris and ind_iri is not None:
			found.append((ind_iri, cls_iri))
	return found
...
class EBSD2RDFPipeline:
	...
	def filter_texture(self, level: str, output_path: Path) -> None:
		"""Konclude-pipeline step: filters texture_ttl down to `level`
		("semi" or "full", see FILTER_LEVELS) and writes the result to
		output_path. There is no "none" level here by design -- for the
		unfiltered ABox variant, this step simply isn't run; texture_ttl is
		merged with the TBox directly."""
		source = rdflib.Graph()
		source.parse(str(self.texture_ttl), format="turtle")
		print(f"Loaded {len(source)} triples from {self.texture_ttl}")

		all_grains = {g for g, _, _ in source.triples((None, HAS_RELATIONAL_QUALITY, None))}
		kept = FILTER_LEVELS[level](source)
		kept_grains = {g for g, _, _ in kept.triples((None, HAS_RELATIONAL_QUALITY, None))}
		kept_pairs = len(list(kept.triples((None, HAS_RELATIONAL_QUALITY, None))))
		print(f"[{level}] Kept {len(kept_grains)}/{len(all_grains)} grains "
		      f"({kept_pairs} misorientation-angle pairs, {len(kept)} triples total)")

		# Carry over the ideal-texture individuals' own rdf:type triples --
		# needed so HermiT/Konclude can see the "towards some <IDEAL_X>"
		# restriction is satisfied, same reason extract_texture_subgraph()
		# does this.
		for s, p, o in source.triples((None, RDF.type, None)):
			if str(s).startswith(str(EX["ideal_"])):
				kept.add((s, p, o))

		kept.serialize(destination=str(output_path), format="turtle")
		print(f"Wrote {len(kept)} triples to {output_path}")

	def merge_for_konclude(self, output_path: Path, tbox_path: Path = DEFAULT_TBOX_PATH, source_path: Optional[Path] = None) -> None:
		"""Konclude-pipeline step: merges a TBox (RDF/XML, e.g.
		KGs/tbox/minimal_module.owl) with an ABox Turtle file into a single
		Turtle file, for Konclude's sparqlfile mode -- unlike HermitReasoner's
		owlready2 setup, which loads the TBox (from a URL) and ABox (from a
		local file) as two separate documents, Konclude parses one ontology
		file and expects everything in it.

		The source ABox's own owl:imports/owl:Ontology annotation (added by
		_dump_abox()/filter_texture()) is dropped before merging -- it's now
		redundant (the TBox is inlined, not imported) and, more importantly,
		having it in the merged file gives Konclude/Redland a live URL it
		might try to resolve at load time, which is something to avoid on a
		compute node without outbound network access."""
		source_path = Path(source_path) if source_path is not None else self.texture_ttl

		g = rdflib.Graph()
		print(f"Loading TBox from {tbox_path} ...")
		g.parse(str(tbox_path), format="xml")
		print(f"  {len(g)} triples after TBox")

		print(f"Loading ABox from {source_path} ...")
	...
	def merge_konclude_results(self, konclude_output_path: Path, output_path: Path, full_abox_path: Optional[Path] = None) -> None:
		"""Konclude-pipeline step: merges Konclude's realisation output
		(OWL/XML, ClassAssertion facts only) back into the full crystallite
		ABox (crystallites_ttl, which has the area triples texture_ttl
		doesn't), producing a reasoned_ttl usable by
		build_populations_and_fractions() -- the Konclude-path equivalent of
		what reason() already does automatically for HermiT."""
		full_abox_path = Path(full_abox_path) if full_abox_path is not None else self.crystallites_ttl

		classifications = extract_grain_type_assertions(str(konclude_output_path))
		print(f"Found {len(classifications)} GRAIN_TYPES classifications in {konclude_output_path}")

		g = rdflib.Graph()
		g.parse(str(full_abox_path), format="turtle")
		print(f"Loaded {len(g)} triples from {full_abox_path}")

		added = 0
		for ind_iri, cls_iri in classifications:
			triple = (URIRef(ind_iri), RDF.type, URIRef(cls_iri))
			if triple not in g:
				g.add(triple)
				added += 1
		print(f"Added {added} new type triples (of {len(classifications)} found -- rest were already present)")
...
def main() -> None:
	...
	parser.add_argument(
		"--filter-level",
		choices=sorted(FILTER_LEVELS),
		default=None,
		help="Required for --stage filter: 'semi' (grain-level) or 'full' (pair-level) filtering strictness.",
	)
	parser.add_argument(
		"--filtered-ttl",
		default=None,
		help="Output of --stage filter (default: KGs/abox/abox_<filter-level>.ttl).",
	)
	parser.add_argument(
		"--tbox-path",
		default=str(DEFAULT_TBOX_PATH),
		help="TBox (RDF/XML) to merge with an ABox for --stage merge-tbox.",
	)
	parser.add_argument(
		"--merge-source",
		default=None,
		help="ABox ttl to merge with --tbox-path for --stage merge-tbox (default: --texture-ttl).",
	)
	parser.add_argument(
		"--merged-ttl",
		default=None,
		help="Required for --stage merge-tbox: output path for the merged TBox+ABox file.",
	)
	parser.add_argument(
		"--konclude-output",
		default=None,
		help="Required for --stage merge-konclude: Konclude realisation's .owl output to merge back into an ABox.",
	)
	parser.add_argument(
		"--stage",
		choices=["crystallites", "filter", "merge-tbox", "reason", "classify-direct", "merge-konclude", "populations", "all"],
		default="all",
		help=(
			...
			"'filter' / 'merge-tbox' / 'merge-konclude' are the Konclude-path steps: filter texture_ttl "
			"to a strictness level, merge the (filtered or unfiltered) ABox with a TBox for Konclude to "
			"consume, then merge Konclude's realisation output back into crystallites_ttl -- point "
			"'populations' at the result via --reasoned-ttl to build the final graph, same as the HermiT path."
		),
	)
	...
	if args.stage == "crystallites":
		...
		if not args.filter_level:
			parser.error("--stage filter requires --filter-level {semi,full}")
		filtered_ttl = Path(args.filtered_ttl) if args.filtered_ttl else ABOX_DIR / f"abox_{args.filter_level}.ttl"
		pipeline.filter_texture(args.filter_level, filtered_ttl)
	elif args.stage == "merge-tbox":
		if not args.merged_ttl:
			parser.error("--stage merge-tbox requires --merged-ttl")
		source_path = Path(args.merge_source) if args.merge_source else None
		pipeline.merge_for_konclude(Path(args.merged_ttl), tbox_path=Path(args.tbox_path), source_path=source_path)
	elif args.stage == "reason":
		pipeline.reason()
	elif args.stage == "classify-direct":
		pipeline.classify_directly()
	elif args.stage == "merge-konclude":
		if not args.konclude_output:
			parser.error("--stage merge-konclude requires --konclude-output")
		pipeline.merge_konclude_results(Path(args.konclude_output), pipeline.reasoned_ttl)
